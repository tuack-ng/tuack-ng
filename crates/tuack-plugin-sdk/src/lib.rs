//! tuack-ng 处理器/渲染器插件的 Rust SDK。
//!
//! 处理器插件实现 [`Processor`]，渲染器插件实现 [`Renderer`]；
//! 分别用 [`processor`] / [`renderer`] 宏注册为 extism 导出函数，
//! JSON 编解码、内存与错误处理均由 SDK 接管。
//!
...
pub use log;
pub use tuack_lib::ren::{CommandResult, ProcessorOutput, RenderDocument, RendererOutput};
...
/// 将错误回传给宿主（供 [`processor`] / [`renderer`] 宏内部使用）。
#[doc(hidden)]
